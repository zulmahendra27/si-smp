...
<div class="page-header">
  <div class="row align-items-end">
    <div class="col-lg-12 d-flex align-items-center justify-content-between">
      ...
      <a href="?p=absensi" class="btn btn-info"><i class="ik ik-arrow-left-circle"></i>Kembali</a>
    </div>
  </div>
</div>
  ...
  <div class="col-md-12">
    <form action="./control/aksi_insert.php" method="post">
      <div class="card">
        ...
        <?php if (in_array($_SESSION['level'], ['guru'])) : ?>
        <div class="card-body">
...
function changeRombel(e) {
  window.location.href = `?p=absensi_add&r=${e.target.value}`;
}

function changeMapel(e) {
  let rombel = document.getElementById('rombel');

  window.location.href = `?p=absensi_add&r=${rombel.value}&m=${e.target.value}`;
}
...
function changeTanggal(e) {
  ...
  window.location.href = `?p=absensi_add&r=${rombel.value}&m=${mapel.value}&t=${e.target.value}`;
}

function changeSiswa(e) {
  window.location.href = `?p=absensi_add&s=${e.target.value}`;
}

function changeBulan(e) {
  let siswa = document.getElementById('siswa');

  window.location.href = `?p=absensi_add&s=${siswa.value}&b=${e.target.value}`;
}

function changeBulanSiswa(e) {
  window.location.href = `?p=absensi_add&b=${e.target.value}`;
}

function changeTahunSiswa(e) {
  let bulan = document.getElementById('bulan');

  window.location.href = `?p=absensi_add&b=${bulan.value}&th=${e.target.value}`;
}
...
function changeSiswaAdmin(e) {
  ...
  window.location.href = `?p=absensi_add&r=${rombel.value}&m=${mapel.value}&s=${e.target.value}`;
}
...
function changeMapelAdmin(e) {
  ...
  window.location.href = `?p=absensi_add&r=${rombel.value}&m=${e.target.value}&s=${siswa.value}`;
}
</script>